...
def is_movies_collection_empty():
    """
    Check if active user´s movie collection is empty

    :return: True or False
    """
    collection = storage.list_movies().fetchone()
...
def command_list_movies():
    ...
    if is_movies_collection_empty():
        print("")
        print(f"{sd.get("name")}, your movie collection is empty. {BOLD}Add some{RESET} movies!")
    else:
...
def command_generate_stats():
    ...
    if not is_movies_collection_empty():
...
def command_generate_website():
    ...
    if not is_movies_collection_empty():
        with open("_static/index.html", "w") as f:
            f.write(
                mcg.load_template_file().replace("__TEMPLATE_TITLE__", "My movies app").replace(
...
def print_movies_menu():
    """
    Print the movies menu
    """
    user_choice = int()
    funct_dict = {0: command_exit, 1: command_list_movies, 2: command_add_movie,
    ...
    print(f"Welcome back, {sd.get("name")}!", end='\n\n')
    while True:
        menu()
        try:
